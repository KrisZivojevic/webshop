...
import { getCategories, getProducts, getProductsByCategory } from "../../api/productsService";
  ...
  return (
    <div className={`max-width ${classes.products__container}`}>
      <ul className={classes.categories__list}>
        <li className={classes.categories__item} onClick={productsHandler}>all</li>
        {categories.map((category) => {
        return (
          <li key={category} className={classes.categories__item} onClick={() => productsByCategoryHandler(category)}>{category}</li>
        )
      })}</ul>
